...
TEfficiency* eff_vs_DC(TTree* summary, TString title){
  const int NBINS = 26; 
  int xlow = 0; 
  int xup = 26; 
...
TProfile* prof_flux_vs_DC(TTree* summary, TString title){
...
void draw(TString label) {
  ...
  TString figfile = Form("%s/effvsdc_flux_%s.pdf", figpath.Data(), label.Data()); 
  TString rootfile = Form("%s/effvsdc_flux_%s.root", figpath.Data(), label.Data()); 
  ...
  TEfficiency* pEff = eff_vs_DC(summary0, title);
  ...
  // flux 
  summary0 = (TTree*) _file0->GetObjectChecked("summary", "TTree");
  TProfile *hprof = prof_flux_vs_DC(summary0, title); 
  hprof->Draw(); 
...
int main(int argc, char** argv) {
  ...
  if ( label != "v1" and label != "roc2" ) {
    cerr << "Support labels: v1, roc2" << endl; 
    return 1 ; 
  }
